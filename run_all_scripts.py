...
class load_infile():
            ...
            if len(line.split()) > 1 and ('predictions were generated by MoorDyn' not in line): # skip blank lines
                if (i == 0):
                    ...
                    for entry in line2.split():      # loop over the elemets, split by whitespace
                        if '***' in entry:
                            if printed == False:
                                print('*** found in {}. Defaulting to 0.0'.format(dirName+fileName))
                                printed = True
                            entry = 0.0   
    ...
    def plot_channel(self, ax, color, channelnum = 1):
        ...
        if self.version == 'dev2':
            pattern = "-"
        elif self.version == 'C' or self.version == 'Cpy':
            pattern = "--"
    ...
    def figures(self, control = "dev2", plot_args = {}, versions = {}, tMax = 0):
        ...
        if ten_rmse or plot_channels or lines_and_tens:
            ...
            self.con_data, self.con_ch, self.con_channels, self.con_units = self.read_mooring_file(self.out_dirname, self.rootname+self.control+'.out')
        ...
        for version in version_list:
            ...
            if self.control not in version:
                ...
                if ten_rmse or plot_channels or lines_and_tens:
                    if version == 'v1':
                        self.test_data, self.test_ch, self.test_channels, self.test_units = self.read_mooring_file(self.out_dirname, 'Line_Lines.out')
                    else:
                        self.test_data, self.test_ch, self.test_channels, self.test_units = self.read_mooring_file(self.out_dirname, self.rootname+version+'.out')
                if line_rmse:
